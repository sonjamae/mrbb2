...
class Map {
	...
	public function setup( $atts ) {
        ...
        if( isset( $atts['regions'] ) && $main_meta ){
			$json_meta = json_decode( '[' . $atts['regions'] . ']', true );
			if( json_last_error() === 0 ){
                if( !is_array( $main_meta['regions'] ) ) {
                    $main_meta['regions'] = [];
                }
        ...
        if( isset( $atts['roundmarkers'] ) && $main_meta ){
            
			$json_meta = json_decode( '[' . $atts['roundmarkers'] . ']', true );
			if( json_last_error() === 0 ){
                if( ! is_array( $main_meta['roundMarkers'] ) ) {
                    $main_meta['roundMarkers'] = [];
                }
	...
	public function render( $atts, $core ) {
		...
		$heightMobile = isset( $this->meta['visual']['paddingTopMobile'] ) ? $this->meta['visual']['paddingTopMobile'] : '';

		// if percentage sign was included, remove it
		$height       = strpos( $height, '%' ) !== false ? str_replace( '%', '', $height ) : $height;
		$heightMobile = strpos( $heightMobile, '%' ) !== false ? str_replace( '%', '', $heightMobile ) : $heightMobile;
